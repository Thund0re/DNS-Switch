...
# ─────────────────────────────────────────────────────────────────────────────
#  DPI / SCALE  — computed once before Tk is created; everything sizes from SC
# ─────────────────────────────────────────────────────────────────────────────

def _detect_scale() -> float:
    """
    Return a scale multiplier relative to a 96-DPI (100 %) baseline.
    1.0 = normal, 1.25 = 125 %, 1.5 = 150 %, 2.0 = 200 % (4K).
    Kept simple: one number, clamped to [0.9, 2.5].
    """
    try:
        if sys.platform == "win32":
            # Per-monitor DPI via SetProcessDpiAwarenessContext then GetDpiForSystem
            ctypes.windll.shcore.SetProcessDpiAwareness(1)   # PROCESS_SYSTEM_DPI_AWARE
            dpi = ctypes.windll.user32.GetDpiForSystem()
            return max(0.9, min(2.5, dpi / 96.0))
    except Exception:
        pass
    # Fallback: use a temporary Tk to read screen geometry
    try:
        import tkinter as _tk
        _r = _tk.Tk(); _r.withdraw()
        sw = _r.winfo_screenwidth()
        _r.destroy()
        # Heuristic: > 2560 px wide → likely HiDPI
        if sw >= 3840: return 2.0
        if sw >= 2560: return 1.5
        if sw >= 1920: return 1.0
        return 0.9
    except Exception:
        return 1.0

# Global scale factor — all font sizes and paddings multiply by this
SC: float = _detect_scale()

def sf(base: int) -> int:
    """Scale a font point size or pixel count by SC, minimum 1."""
    return max(1, round(base * SC))

def sp(base: int) -> int:
    """Scale a padding value; result is at least 1."""
    return max(1, round(base * SC))
...
async def startup_probe():
    log("── Startup probe ─────────────────────────────────", "act")
    log("  [1/3] Detecting active network interface…", "info")
    ...
    if not iface:
        ...
        log("  [1/3] No active interface found", "err")
    else:
        S.active_interface = iface
        log(f"  [1/3] Interface: {iface}", "ok")

        log("  [2/3] Reading current DNS settings…", "info")
        mode, dns_list = await get_current_dns(iface)
        S.current_dns_mode = mode
        S.current_dns_list = dns_list
        dns_str = "DHCP" if mode == "dhcp" else ("  ".join(dns_list) if dns_list else "unknown")
        log(f"  [2/3] Current DNS: {dns_str}", "ok")
    ...
    log("  [3/3] Fetching public WAN IP…", "info")
    wan = await wan_task
    S.wan_ip = wan
    log(f"  [3/3] WAN IP: {wan}", "ok")
    def _upd_wan():
...
async def _do_apply(primary: str, secondary: str, name: str = "Custom"):
    if not _demand_admin():
        return
    pulse(True)
    log(f"Apply DNS [{name}]  {primary} / {secondary}", "act")
    log(f"  [1/3] Reading current DNS settings…", "info")
    set_status("Reading current DNS…", T["warn"])
    S.previous_mode, S.previous_dns = await get_current_dns(S.active_interface)
    log(f"  [1/3] Previous: {S.previous_mode}  {S.previous_dns}", "info")
    log(f"  [2/3] Setting primary  → {primary}", "info")
...
async def _do_reset():
    if not _demand_admin():
        return
    pulse(True)
    log("Reset DNS → DHCP (Automatic)", "act")
    log("  [1/2] Saving current settings…", "info")
    set_status("Saving current DNS…", T["warn"])
    S.previous_mode, S.previous_dns = await get_current_dns(S.active_interface)
...
async def _do_undo():
    ...
    if not S.previous_mode:
        log("Nothing to undo — no previous DNS recorded", "warn")
        ui(_msgwarn, "Undo", "Nothing to restore."); return
    pulse(True)
    log("Undo — restoring previous DNS", "act")
    log(f"  [1/2] Previous was: {S.previous_mode}  {S.previous_dns}", "info")
...
async def _do_flush():
    if not _demand_admin():
        return
    pulse(True)
    log("Flush DNS resolver cache", "act")
    log("  [1/2] Running ipconfig /flushdns…", "info")
    set_status("Flushing DNS cache…", T["accent"])
    try:
        out = await arun("ipconfig /flushdns")
        # ipconfig echoes a success line — log it verbatim if present
        for line in out.splitlines():
            l = line.strip()
            if l:
                log(f"  [2/2] {l}", "info")
...
async def _do_speed_test():
    ...
    set_status(f"Speed test  0/{total}…", T["warn"])
    log(f"── DNS Speed Test — {total} providers ─────────────────", "act")
    ...
    # Build named tasks so we can report progress as each one finishes
    named_tasks = {
        asyncio.ensure_future(test_one(n, ips[0])): n
        for n, ips in DNS_PROVIDERS.items()
    }

    results = []
    pending = set(named_tasks.keys())
    ...
    while pending:
        ...
        for fut in finished:
            ...
            set_status(f"Speed test  {done[0]}/{total}…", T["warn"])
            if p is not None and d is not None:
                score = round(p * 0.6 + d * 0.4)
                results.append((name, round(p), round(d), score))
                # Log result immediately as it arrives
                short = name.split("—")[0].strip()[:22]
                log(f"  [{done[0]:>2}/{total}] {short:<22}  ping {round(p):>4}ms  "
                    f"resolve {round(d):>4}ms  score {score}ms", "info")
            else:
                short = name.split("—")[0].strip()[:22]
                log(f"  [{done[0]:>2}/{total}] {short:<22}  unreachable", "warn")
    ...
    log(f"  ✓ Done — fastest: {fastest[0]}  ({fastest[3]}ms score)", "ok")
...
def flat_btn(parent, text, cmd, bg, fg="#FFFFFF", font=None,
    ...
    b = tk.Button(parent, text=text, command=cmd,
                  ...
                  activeforeground=fg,
                  font=font or ("Segoe UI", sf(9), "bold"),
                  relief="flat", bd=0,
...
def _section_label(parent, text: str):
    ...
    f.pack(fill="x", pady=(0, sp(4)))
    tk.Label(f, text=text, font=("Segoe UI", sf(7), "bold"),
             bg=T["bg"], fg=T["fg2"]).pack(side="left")
    tk.Frame(f, height=1, bg=T["border"]).pack(
        side="left", fill="x", expand=True, padx=(sp(6), 0), pady=sp(3))
...
def _build_ui(restore_combo: str = None):
    ...
    style.configure("DNS.TCombobox",
        ...
        padding=(sp(10), sp(7)), font=("Segoe UI", sf(9)))
    ...
    hi.pack(fill="x", padx=sp(18), pady=sp(10))

    # ── Title row ──────────────────────────────────────────────────────────
    tr = tk.Frame(hi, bg=T["surface"])
    tr.pack(fill="x")
    tk.Label(tr, text="bork", font=("Segoe UI", sf(18), "bold"),
             bg=T["surface"], fg=T["accent"]).pack(side="left")
    tk.Label(tr, text="DNS", font=("Segoe UI", sf(18), "bold"),
             bg=T["surface"], fg=T["fg"]).pack(side="left")
    tk.Label(tr, text="  v0.3", font=("Segoe UI", sf(10)),
             bg=T["surface"], fg=T["fg2"]).pack(side="left", pady=(sp(4), 0))
    indicator = tk.Label(tr, text="  ●", font=("Segoe UI", sf(11)),
                         bg=T["surface"], fg=T["border"])
    indicator.pack(side="left", pady=(sp(3), 0))
    ...
    for key, sym in (("dark", "◑"), ("light", "○"), ("tinted", "◐")):
        ...
        b = tk.Label(thm_fr, text=sym, font=("Segoe UI", sf(13)),
                     bg=T["surface"], fg=c, cursor="hand2")
        b.pack(side="left", padx=sp(4))
    ...
    sr.pack(fill="x", pady=(sp(2), 0))
    tk.Label(sr, text="STATUS", font=("Segoe UI", sf(7), "bold"),
    ...
    status_val = tk.Label(sr, text="Initialising…",
                          font=("Segoe UI", sf(9)), bg=T["surface"], fg=T["warn"])
    status_val.pack(side="left")
    wan_ip_lbl = tk.Label(sr, text=f"WAN  {S.wan_ip}",
                          font=("Consolas", sf(9)), bg=T["surface"], fg=T["accent"])
    wan_ip_lbl.pack(side="right", padx=(0, sp(2)))

    # Admin badge
    adm_col  = T["ok"] if S.admin else T["warn"]
    adm_text = "● ADMIN" if S.admin else "○ limited mode"
    tk.Label(sr, text=f"  │  {adm_text}", font=("Segoe UI", sf(7), "bold"),
             bg=T["surface"], fg=adm_col).pack(side="right", padx=(0, sp(8)))

    # ── BIG CURRENT DNS BANNER ─────────────────────────────────────────────
    dns_card = tk.Frame(hi, bg=T["surface2"],
                        highlightthickness=1, highlightbackground=T["border"])
    dns_card.pack(fill="x", pady=(sp(8), sp(2)))
    dns_inner = tk.Frame(dns_card, bg=T["surface2"])
    dns_inner.pack(fill="x", padx=sp(14), pady=sp(10))

    tk.Label(dns_inner, text="ACTIVE DNS", font=("Segoe UI", sf(7), "bold"),
    ...
    current_dns_primary_lbl = tk.Label(
        dns_ips, text="…",
        font=("Consolas", sf(22), "bold"),
        bg=T["surface2"], fg=T["accent"])
    current_dns_primary_lbl.pack(side="left")

    tk.Label(dns_ips, text="  /  ", font=("Consolas", sf(16)),
    ...
    current_dns_name_lbl = tk.Label(
        ...
        font=("Segoe UI", sf(8)),
        bg=T["surface2"], fg=T["fg2"])
    current_dns_name_lbl.pack(anchor="w", pady=(sp(2), 0))
    ...
    left.pack(side="left", fill="both", padx=(sp(16), sp(8)), pady=sp(14))
    ...
    ic_inner.pack(fill="x", padx=sp(10), pady=sp(8))
    tk.Label(ic_inner, text="PRIMARY", font=("Segoe UI", sf(7), "bold"),
             bg=T["surface2"], fg=T["fg2"]).grid(row=0, column=0, sticky="w")
    primary_lbl = tk.Label(ic_inner, text="", font=("Consolas", sf(11), "bold"),
                           bg=T["surface2"], fg=T["accent"])
    primary_lbl.grid(row=1, column=0, sticky="w")
    tk.Frame(ic_inner, width=sp(20), bg=T["surface2"]).grid(row=0, column=1, rowspan=2)
    tk.Label(ic_inner, text="SECONDARY", font=("Segoe UI", sf(7), "bold"),
             bg=T["surface2"], fg=T["fg2"]).grid(row=0, column=2, sticky="w")
    secondary_lbl = tk.Label(ic_inner, text="", font=("Consolas", sf(11), "bold"),
                              bg=T["surface2"], fg=T["fg2"])
    ...
    right.pack(side="left", fill="both", expand=True, padx=(sp(8), sp(16)), pady=sp(14))

    lh = tk.Frame(right, bg=bg)
    lh.pack(fill="x", pady=(0, sp(6)))
    tk.Label(lh, text="ACTIVITY LOG", font=("Segoe UI", sf(7), "bold"),
             bg=bg, fg=T["fg2"]).pack(side="left")
    flat_btn(lh, "clear", lambda: _clear_log(),
             bg, fg=T["fg2"], padx=sp(6), pady=sp(2),
             font=("Segoe UI", sf(7))).pack(side="right")
    ...
    log_box = tk.Text(lf, bg=T["log_bg"], fg=T["fg"],
                      ...
                      padx=sp(10), pady=sp(8), wrap="word",
...
def _restore_geometry(prefs: dict):
    ...
    if g:
        try:
            ...
            pass
    # Base size at 1x; scale by SC so the window fills a similar portion of screen
    base_w = int(820 * SC)
    base_h = int(620 * SC)
    w = min(max(base_w, int(sw * 0.52)), int(sw * 0.92))
    h = min(max(base_h, int(sh * 0.62)), int(sh * 0.92))
...
def main():
    ...
    root = tk.Tk()
    root.title("borkDNS v0.3")
    root.configure(bg=T["bg"])
    root.resizable(True, True)
    root.minsize(sf(700), sf(560))
    ...
    log(f"borkDNS v0.3 started  scale={SC:.2f}x" + ("  [ADMIN]" if S.admin else "  [limited]"), "act")
